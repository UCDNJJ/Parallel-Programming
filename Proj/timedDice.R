...
# Marks a checkpoint into the data
checkpoint <- function(marker, message) {
  newMarker <- Sys.time()
  # Calculate difference
  difference <- newMarker - marker
  units(difference) <- "secs"
  # Store into data
  times <<- c(times, difference)
  markers <<- c(markers, message)
  return(newMarker)
}
...
getEventProb = function(nrolls,
                        ...
                        topCall=FALSE)
{
  # Mark beginning of function call
  marker <- Sys.time()
  ...
  if(topCall) marker <- checkpoint(marker, 'Setup')
  ...
  else # i.e., if (!orderMatters)
  {
    ...
    if (maxListElemLength > 1)
    {
      ...
      if(topCall) marker <- checkpoint(marker, 'Build Combination Matrix')
      ...
      if(topCall) marker <- checkpoint(marker, 'Build Unique Matrix')
      ...
      if(topCall) marker <- checkpoint(marker, 'Calculate Probabilities')
    ...
    else
    {
      ...
      if(topCall) marker <- checkpoint(marker, 'Calculate Probability')
    }
  }
...
# Run function to time
getEventProb(nr=3, nd=2, ns=6, list(2, 3:5, 3), orderMatters=FALSE, topCall=TRUE)
# Combine into data frame
df <- data.frame(time=times, marker=markers, stringsAsFactors = FALSE)
# Plot into bar chart
(ggplot(df, aes(x = factor(marker), y = time))
